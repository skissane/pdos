...
/*Function to dump the contents of DS,DX register*/
        ...
        case 0xf6:
            ...
            else if (regsin->h.al == 0x45)
            {
                regsout->d.eax = PosScrncap(regsin->d.edx);
            }
#endif
#ifndef __32BIT__
            else if (regsin->h.al == 0x42)
            ...
            {
                regsout->x.ax = PosScrncap(regsin->x.dx);
            }
            ...
            else if (regsin->h.al == 0x4B)
            {
                regsout->x.ax = PosScreenMap(MK_FP(sregs->ds,regsin->x.bx));
            }
            ...
            {
                regsout->d.eax = PosScreenMap((unsigned char *)regsin->d.ebx);
            }
            else if (regsin->h.al == 0x4C)
            {
