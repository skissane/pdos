                            ...
                            IMAGE_IMPORT_DESCRIPTOR *import_desc)
{
    ...
    if (optional_hdr->NumberOfRvaAndSizes > DATA_DIRECTORY_IMPORT_TABLE)
    {
        ...
        if (data_dir->Size != 0)
        {
            ...
            for (; import_desc->OriginalFirstThunk; import_desc++)
            {
                /* this code is repeated and should be consolidated */
#ifdef W32DLL
                if (strcmp(dllStart + import_desc->Name, "kernel32.dll") == 0)
                {
                unsigned long *thunk;

                for (thunk = (void *)(dllStart + (import_desc->FirstThunk));
                     *thunk != 0;
                     thunk++)
                {
                    if ((*thunk) & 0x80000000UL)
                    ...
                    {
                        /* Import by name. */
                        unsigned char *hintname = dllStart + ((*thunk) & 0x7fffffffUL);
                        int i;
                ...
                {
                /* use braces so that we can go ret = exe... later */
#endif
