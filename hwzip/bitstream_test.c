...
{
        istream_t is;
        static const uint8_t bits = 0x47; /* 0100 0111 */
        static const uint8_t arr[9] = { 0x45, 0x48 }; /* 01000101 01001000 */
        ...
        istream_init(&is, arr, 9);
        CHECK(istream_bytes_read(&is) == 0);

        /* Advance 8 bits, one at a time. */
        CHECK(istream_advance(&is, 1)); CHECK(istream_bytes_read(&is) == 1);
        CHECK(istream_advance(&is, 1)); CHECK(istream_bytes_read(&is) == 1);
        CHECK(istream_advance(&is, 1)); CHECK(istream_bytes_read(&is) == 1);
        CHECK(istream_advance(&is, 1)); CHECK(istream_bytes_read(&is) == 1);
        CHECK(istream_advance(&is, 1)); CHECK(istream_bytes_read(&is) == 1);
        ...
        CHECK(istream_advance(&is, 1)); CHECK(istream_bytes_read(&is) == 2);
}

void test_istream_min_bits(void)
{
